...
| S3.19 | duplicate verdicts on the topic | found live (seq 58+59, same room, same verdict twice): two verdict-page pollers raced INSIDE our single server process. Real fix (not the window-shrinking mitigation first proposed): per-room reveal coalescing — the first request creates the in-flight promise, concurrent requests await it; with one process this makes a second concurrent reveal impossible. Plus reader hygiene: take the FIRST verdict on the topic, not the last. | 0G/Hedera | S2.9 | frank | ⬜ (Dylan: Frank to validate the coalescing approach and implement — reveal path is his lane) |
| S3.20 | evaluator robustness to loose phrasing | live probes prove clean inputs work (price-only overlap → workable; miss → gap:single; 4-dims → workable) but a real two-human test with informal phrasing got gap:multiple on an intended match. Harden the prompt counting rule: a dimension blocks ONLY if both positions state incompatible constraints on it; unmentioned or one-sided mentions never block. Add loose-phrasing cases to eval:live. Reproduction pending the testers' verbatim texts. | 0G | S2.2 | frank | ⬜ |
| S5.0 | merge train | **frank/integrator:** merge PR #19 (8 commits: rename, share button, copy pass, invite wiring) into develop, then develop → main for submission. Nothing ships from an unmerged branch. | compliance | — | frank | ⬜ |
